...
class TrackDesc:
    ...
    def __repr__(self):
        return f"TrackDesc(TrackNum={self.TrackNum} BytesRead={self.BytesRead} FirstNoteClick={self.FirstNoteClick} FirstNoteMs={self.FirstNoteMs} LastNoteClick={self.LastNoteClick} LastNoteMs={self.LastNoteMs} LyricsTrack={self.LyricsTrack} text_events={self.text_events} lyrics_events={self.lyric_events})"
...
class LyricSyllable:
    ...
    def __repr__(self):
        return f"LyricSyllable(ms={self.ms} text={self.text})"
...
class Lyrics:
    ...
    def __repr__(self):
        return f"Lyrics(line={self.line} list={self.list})"
...
def midiParseData(midiData, ErrorNotifyCallback, Encoding):
    ...
    for track in midifile.trackList:
        ...
        if earliestNoteMS and track.FirstNoteMs < earliestNoteMS:
            earliestNoteMS = track.FirstNoteMs
        if track.LastNoteMs and not lastNoteMS:
            lastNoteMS = track.LastNoteMs
        if lastNoteMS and track.LastNoteMs > lastNoteMS:
            lastNoteMS = track.LastNoteMs
    ...
    if debug:
        print(f"Song start = {midifile.earliestNoteMS} ms")
        print(f"Song end = {midifile.lastNoteMS} ms")
    ...
    if event_type == 0xFF:
        ...
        if event == 0x00:
            ...
            if Length > 1000:
                ...
                if debug:
                    print(f"Ignoring text of length {Length}")
            else:
            ...
            bytesRead = bytesRead + Length
            if debug:
                print(f"Track Title: {repr(title)}")
            if title == "Words":
            ...
            valid, tempoA, tempoB, tempoC = list(packet)
            if valid != 0x03:
            ...
            if debug:
                ...
                print(f"Tempo: {tempo} ({ms_per_quarter} ms per quarter note)")
        elif event == 0x54:
            # SMPTE (discard)
            ...
            valid, num, denom, clocks, notes = list(packet)
            if valid != 0x04:
        ...
        # Unknown event (discard)
        if debug:
            print(("Unknown event of type: 0x%x" % event_type))
        Length, varBytes = varLength(filehdl)
        bytesRead = bytesRead + varBytes
        _ = filehdl.read(Length)
        bytesRead = bytesRead + Length
...
class midPlayer(pykPlayer):
    def __init__(self, song, songDb, errorNotifyCallback=None, doneCallback=None):
        ...
        if debug:
            print("===== Found lyrics START")
            self.midifile.lyrics.write()
            print("===== Found lyrics END")
